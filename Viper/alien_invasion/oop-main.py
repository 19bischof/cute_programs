...
class Conni(Base):
    ...
    def draw():
        ...
        pygame.draw.rect(Conni.screen, (3, 127, 252), (0, Conni.screen_height *
                         0.96, Conni.screen_width, Conni.screen_height*0.04))  # drawing the sea
        Conni.my_ship.draw()
        Ball.draw()
        Alien.draw()
...
class Alien(Base):
    ...
    width = 32
    speed = 1
    image = pygame.image.load(project_path+"/images/ufo.png").convert_alpha()
...
class Ball(Base):
    radius = 20
    speed = 5
    list_of_colors = [(239, 7, 73), (224, 93, 244), (93, 216, 244),
                      (93, 244, 133), (222, 244, 93), (242, 114, 29)]
    color_index = 0
    pre_image = pygame.image.load(project_path+"/images/medicine-ball.png").convert_alpha()
    instances = []
    ...
    def draw():
        Ball.image = pygame.transform.smoothscale(
            Ball.pre_image, (Ball.radius*2, Ball.radius*2))
        for b in Ball.instances:
...
class Ship:
    ...
    pre_image = pygame.image.load(project_path+"/images/ship.png").convert_alpha()
    ...
    def draw(self):
        Ship.image = pygame.transform.scale(
            Ship.pre_image, (Ship.width, Ship.height))
