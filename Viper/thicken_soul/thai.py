...
# bot has to return when making a move!!
import os
...
cons = True #if print to console
no_early_finish_check = False #skip check if game still winnable (time_consuming)
log_it = True #enable logging 
def input_place_sign():
    # user method for placing sign
    global p_index
    stopping = False
    if log_it: logging.info("Player{} is choosing with input".format(p_index + 1))
    while not stopping:
        os.system("cls" if os.name == "nt" else "clear")
        try:
            if log_it: logging.info("getting input from user")
            inp = (
            ...
            quit()
        if inp in ("?", "help"):
            if log_it: logging.debug("calling help function")
            print_help()
            input("Press Enter to continue!")
            continue
        if log_it: logging.debug("raw input is " + inp)
        for x_pos in x_names:
            if inp.find(x_pos) != -1:
                if log_it: logging.debug("found x position in input: " + x_pos)
                for y_pos in y_names:
                    if inp.find(y_pos) != -1:
                        if log_it: logging.debug("found y position in input: " + y_pos)
                        if grid[y_names.index(y_pos)][x_names.index(x_pos)]:
                            if log_it: logging.debug(
...
def finished():
    ...
    if log_it: logging.debug("check for winner")
    if cons:print(attr("reset"), end="", flush=True)
    # check for winner
    for i in range(3):
        if grid[i][0] == grid[i][1] == grid[i][2] and grid[i][0]:
            if cons:print(pre[grid[i][0] - 1] + " won the game!" + attr("reset"))
            if log_it: logging.info("Player{} won".format(grid[i][0]))
            return "Player{} won".format(grid[i][0])
        if grid[0][i] == grid[1][i] == grid[2][i] and grid[0][i]:
            if cons:print(pre[grid[0][i] - 1] + " won the game!" + attr("reset"))
            if log_it: logging.info("Player{} won".format(grid[0][i]))
            return "Player{} won".format(grid[0][i])
    if grid[0][0] == grid[1][1] == grid[2][2] and grid[0][0]:
        if cons:print(pre[grid[0][0] - 1] + " won the game!" + attr("reset"))
        if log_it: logging.info("Player{} won".format(grid[0][0]))
        return "Player{} won".format(grid[0][0])
    if grid[2][0] == grid[1][1] == grid[0][2] and grid[2][0]:
        if cons:print(pre[grid[2][0] - 1] + " won the game!" + attr("reset"))
        if log_it: logging.info("Player{} won".format(grid[2][0]))
        return "Player{} won".format(grid[2][0])
    if log_it: logging.debug("check for possibility to win")
    ...
    if no_early_finish_check:
        if one_row_grid.count(0) == 0:
            return "no one wins"
        return False
        ...
        if diags[i].count(0) != 0:
            if (diags[i].count(1), diags[i].count(2)) != (1, 1):
                if one_row_grid.count(0) == diags[i].count(0) != 1:
                    ...
                    return False
    if cons:print("No one wins!")
    if log_it: logging.info("no possibility to win")
    return "no one wins"
...
def func_place_sign(c: tuple[int]):  # c[0] is y; c[1] is x
    ...
    global p_index
    if cons:clear_screen()
    if grid[c[0]][c[1]]:
        logging.error("Coordinates {} already taken".format(c))
        raise ValueError
    grid[c[0]][c[1]] = p_index + 1
    history.append((c[0], c[1]))
    if cons:print(pre[p_index] + " sets it at {} {}".format(y_names[c[0]], x_names[c[1]]))
    if log_it: logging.info("function set sign at ->{}<-".format(repr(c)))
    p_index = (p_index + 1) % 2


def bot0_angela():
    # function: make random move
    if log_it: logging.info("Player{} is choosing with bot0_angela".format(p_index + 1))
    while True:
...
def bot1_amelie():
    ...
    if log_it: logging.info("Player{} is choosing with bot1_amelie".format(p_index + 1))
    calc = {0: 2, 1: 0, 2: -2}
    try:
        y, x = history[-1]
        if grid[y][x]:
            ...
            if not grid[mir_y][mir_x]:
                ...
                return
            if log_it: logging.debug("spot already taken " + repr((mir_y, mir_x)))
    except IndexError:
        pass
    if log_it: logging.debug("unable to find a move")
    bot0_angela()
...
def bot2_aurora():
    ...
    if log_it: logging.info("Player{} is choosing with bot2_aurora".format(p_index + 1))
    ...
    for c in corners:
        ...
        if corner_score > best_corner_score:
            if log_it: logging.debug("corner_score updated to {}".format(corner_score))
            best_corner = c
...
def bot3_anasthasia():
    ...
    if log_it: logging.info("Player{} is choosing with bot3_anasthasia".format(p_index + 1))
    ...
    for g in (grid, transp_grid):
        for y in range(3):
            if g[y].count(p_index + 1) == 2 and g[y].count(0) == 1:
                if log_it: logging.debug("found winning move!")
                for x in range(3):
                    if not g[y][x]:
                        if g == transp_grid:
                            func_place_sign((x, y))
    ...
    for g in (grid, transp_grid):
        for y in range(3):
            if g[y].count(next_p_index + 1) == 2 and g[y].count(0) == 1:
                if log_it: logging.debug("found enemy's winning move!")
                for x in range(3):
                    if not g[y][x]:
                        if g == transp_grid:
                            func_place_sign((x, y))
...
def bot4_ashley():
    ...
    if log_it: logging.info("Player{} is choosing with bot4_ashley".format(p_index + 1))
    corners = ((0, 0), (2, 0), (2, 2), (0, 2))
    ...
    for i, c in enumerate(corners):
        if history == [c, (1, 1), corners[i - 2]]:
            ...
            return
    if history == []:
        func_place_sign(random.choice(corners))
        return
    if len(history)==2 and history[0] in corners and history[1] == (1,1):
        func_place_sign(corners[corners.index(history[0])-2])
        return
    if log_it: logging.debug("unable to find a move")
    bot3_anasthasia()
...
def reset_game():
    if log_it: logging.info("#################### resetting game ##################################")
    global grid,history,p_index
    grid = [[0] * 3 for x in range(3)]  # 0:empty, 1:player1, 2:player2
    # syntax: grid[y][x]
    history = []
...
if __name__ == "__main__":
    ...
    if log_it: logging.debug("starting Game Loop")
    i = p_index
    while 1:
        player_methods[i]()
        if finished():
            if log_it: logging.debug("Break out of Game Loop")
            break
