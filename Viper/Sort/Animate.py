...
class Animate:
    def render(list,vip=[],vvip=[]):
        ...
        height = int(st.screen_height / (st.range)) 
        ...
        for index,bar in enumerate(list):
            ...
            if index in vvip:
                color = (39, 26, 232)
            pygame.draw.rect(Window.instance.screen,color,((1+width) * index,(st.screen_height - bar * height)+1,width,bar*height))
        Window.update()
    def start():    
        print(chr(27)+ "[2J")
        selection = ""    
        ...
        if selection in Bubblesort_set:
            Animate.Bubblesort()
        if selection in Insertionsort_set:
            Animate.Insertionsort()
        if selection in Shakersort_set:
            Animate.Shakersort()
